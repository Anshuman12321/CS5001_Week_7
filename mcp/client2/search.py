...
class SearchClient:
    ...
    async def search(self, pattern: str, path: str) -> dict:
        ...
        #   Lines look like "path/to/file.py:42:   def foo():"
        #   Use regex so Windows paths (C:\...) are not split on drive colon.
        path_line = re.compile(r"^(.+\.py):(\d+):(.*)$")
        files: list[str] = []
        for line in raw_matches.splitlines():
            m = path_line.match(line.strip())
            if m:
                filepath = m.group(1)
